...
import { fetchBoxes, createBox, updateBox, deleteBox, createItem, deleteItem, updateItem } from './storage';
...
const PASSPHRASE = 'tissa';

// ─── Auth Modal ───
function AuthModal({ onSuccess, onCancel, actionLabel }) {
  ...
  return (
    <div className="auth-overlay" onClick={onCancel}>
      <div className="auth-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 className="auth-title">需要驗證</h3>
        <p className="auth-desc">請輸入管理密碼以{actionLabel}</p>
        <input ref={ref} className={`input auth-input ${error ? 'auth-error' : ''}`} type="password" placeholder="輸入密碼..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSubmit()} />
        {error && <p className="auth-error-text">密碼錯誤，請重試</p>}
        <div className="auth-actions">
          <button className="btn-ghost" onClick={onCancel}>取消</button>
          <button className="btn-small-primary" onClick={handleSubmit} disabled={!input.trim()}>確認</button>
        </div>
      </div>
    </div>
  );
...
// ─── useAuth hook ───
function useAuth(showToast) {
  const [authed, setAuthed] = useState(false);
  const [pendingAction, setPendingAction] = useState(null);

  const requireAuth = (actionLabel, callback) => {
    if (authed) { callback(); }
    else { setPendingAction({ label: actionLabel, callback }); }
  };

  const handleAuthSuccess = () => {
    setAuthed(true);
    if (pendingAction) { pendingAction.callback(); setPendingAction(null); }
    showToast('已驗證通過 ✓');
  };

  const authModal = pendingAction ? (
    <AuthModal actionLabel={pendingAction.label} onSuccess={handleAuthSuccess} onCancel={() => setPendingAction(null)} />
  ) : null;

  return { authed, requireAuth, authModal };
}
...
export default function App() {
  ...
  const { authed, requireAuth, authModal } = useAuth(showToast);

  const loadBoxes = useCallback(async () => {
    const data = await fetchBoxes(); setBoxes(data); return data;
  }, []);
  ...
  useEffect(() => {
    loadBoxes().then((data) => {
      ...
      if (boxId && data) {
        ...
        if (found) { setSelectedBox(found); setReadonly(true); setView('box'); }
      }
    });
  }, [loadBoxes]);
  ...
  const handleAddBox = async (boxData) => {
    ...
    if (newBox) { await loadBoxes(); showToast('箱子已新增！'); setView('home'); }
    else { showToast('新增失敗', 'warn'); }
    setSyncing(false);
  };

  const handleDeleteBox = async (boxId) => {
    setSyncing(true); await deleteBox(boxId); await loadBoxes();
    showToast('箱子已刪除', 'warn'); setView('home'); setSyncing(false);
  };

  const handleAddItem = async (boxId, itemData) => {
    setSyncing(true);
    let photoFile = null;
    if (itemData.photo) photoFile = toBlobUtil(itemData.photo);
    const result = await createItem(boxId, { ...itemData, photoFile });
    if (result) { await loadBoxes(); showToast('物品已新增！'); }
    else { showToast('新增失敗', 'warn'); }
    setSyncing(false);
  };

  const handleDeleteItem = async (itemId) => {
    setSyncing(true); await deleteItem(itemId); await loadBoxes(); setSyncing(false);
  };

  const handleUpdateItem = async (itemId, updates) => {
    setSyncing(true); await updateItem(itemId, updates); await loadBoxes(); setSyncing(false);
  };

  const handleUpdateBox = async (boxId, updates) => {
    await updateBox(boxId, updates); await loadBoxes();
  };

  const openBox = (box) => { setSelectedBox(box); setReadonly(false); setView('box'); };
  const openBoxReadonly = (box) => { setSelectedBox(box); setReadonly(true); setView('box'); };

  const searchResults = search.trim()
    ? boxes.reduce((acc, box) => {
        const s = search.toLowerCase();
        const mi = (box.items || []).filter((i) => i.name.toLowerCase().includes(s) || (i.note && i.note.toLowerCase().includes(s)));
  ...
  const handleQRResult = useCallback((value) => {
    ...
    try {
      ...
      if (boxId) { const f = boxes.find((b) => b.id === boxId); if (f) { openBoxReadonly(f); showToast(`已找到：${f.name}`); return; } }
    } catch {}
    const f = boxes.find((b) => b.id === value);
    if (f) { openBoxReadonly(f); showToast(`已找到：${f.name}`); return; }
  ...
  return (
    <div className="app">
      ...
      <Toast toast={toast} />
      {authModal}
      {syncing && <div className="sync-bar">同步中...</div>}

      <header className="header">
        {view !== 'home' && (
          <button className="back-btn" onClick={() => { setView('home'); setSearch(''); }}><Icons.Back /></button>
        )}
        <div style={{ flex: 1 }}>
          <h1 className="title">
            {view === 'home' && '📦 入庫管理'}
          ...
          {view === 'home' && (
            <span className="subtitle">
              {boxes.length} 個箱子 · ☁️ 雲端同步
              {authed && <span style={{ color: '#10B981' }}> · 🔓 已驗證</span>}
            </span>
          )}
        </div>
      </header>
      ...
      {view === 'home' && (
        <div className="content">
          ...
          <div className="action-row">
            <button className="btn-primary" onClick={() => requireAuth('新增箱子', () => setView('add'))}><Icons.Plus /> 新增箱子</button>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {matchItems.length > 0 && (
                      <div className="match-items">
                        {matchItems.map((i) => <span key={i.id} className="match-chip">{i.photo_url && <Icons.Image />} {i.name}</span>)}
                      </div>
                    )}
                  </div>
...
// ─── Box Detail ───
function BoxDetailView({ box, baseUrl, onAddItem, onDeleteItem, onUpdateItem, onDeleteBox, onUpdateBox, onImageClick, syncing, requireAuth }) {
    ...
    <div className="content">
      <div className="card box-info" style={{ borderLeft: `4px solid ${box.color || '#3B82F6'}` }}>
        ...
        <div className="qr-section">
          ...
          {showQR && <div style={{ marginTop: 14 }}><QRCodeDisplay boxId={box.id} boxName={box.name} baseUrl={baseUrl} /></div>}
        </div>
      </div>

      {!showAdd ? (
        <button className="add-item-btn" onClick={() => requireAuth('新增物品', () => setShowAdd(true))}><Icons.Plus /> 新增物品</button>
      ) : (
          ...
          {items.map((item, idx) => (
            <div key={item.id} className="item-row">
              ...
              <div className="qty-control">
                <button className="qty-btn" onClick={() => handleQtyChange(item, -1)} disabled={syncing || (item.qty || 1) <= 1}>−</button>
                <span className="qty-value">{item.qty || 1}</span>
                <button className="qty-btn" onClick={() => handleQtyChange(item, 1)} disabled={syncing}>+</button>
              </div>
              <button className="del-btn" onClick={() => requireAuth('刪除物品', () => onDeleteItem(item.id))}><Icons.Trash /></button>
            </div>
          ))}
        </div>
      ) : (
            ...
            <div key={item.id} className="photo-grid-item">
              ...
              <div className="photo-grid-label">
                ...
                <div className="qty-control sm">
                  <button className="qty-btn sm" onClick={() => handleQtyChange(item, -1)} disabled={syncing || (item.qty || 1) <= 1}>−</button>
                  <span className="qty-value">{item.qty || 1}</span>
                  <button className="qty-btn sm" onClick={() => handleQtyChange(item, 1)} disabled={syncing}>+</button>
                </div>
              </div>
              <button className="photo-grid-del" onClick={() => requireAuth('刪除物品', () => onDeleteItem(item.id))}><Icons.Trash /></button>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="danger-zone">
        {!confirmDel ? (
          <button className="danger-btn" onClick={() => requireAuth('刪除箱子', () => setConfirmDel(true))}>刪除此箱子</button>
        ) : (
...
// ─── Box Read-Only View (for QR scan / external link) ───
function BoxReadOnlyView({ box, onImageClick }) {
  const items = box.items || [];
  const hasPhotos = items.some((i) => i.photo_url);
  const [viewMode, setViewMode] = useState('list');
    ...
    <div className="content">
      ...
      <div className="card box-info" style={{ borderLeft: `4px solid ${box.color || '#3B82F6'}` }}>
        ...
        <div style={{ borderTop: '1px solid #334155', paddingTop: 10, marginTop: 10 }}>
          <span className="readonly-badge">👁 瀏覽模式</span>
        </div>
      </div>
      ...
      {/* Items */}
      {items.length === 0 ? (
        <div className="empty-state"><p className="muted">這個箱子是空的</p></div>
      ) : viewMode === 'list' ? (
        <div className="item-list">
          {items.map((item, idx) => (
            <div key={item.id} className="item-row">
              ...
              <span className="qty-readonly">{item.qty || 1}</span>
            </div>
          ))}
        </div>
      ) : (
